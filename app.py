...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)        "📋 Команды:\n"
        "/start — начать\n"
        "/help — помощь\n"
        "/test — тест на тревожность\n"
        "/clear — очистить историю\n"
...
def set_webhook():
    ...
    webhook_url = f"https://psychobot-xl1y.onrender.com/webhook"
    try:
        # Используем asyncio для правильного вызова
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(telegram_app.bot.set_webhook(webhook_url))
        return f"✅ Вебхук установлен: {webhook_url}", 200
...
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 10000))
    logger.info(f"🚀 Запуск Flask на порту {port}...")
    app.run(host="0.0.0.0", port=port)        "/start — начать\n"
        "/help — помощь\n"
        "/test — тест на тревожность\n"
        "/clear — очистить историю\n"
        "/crisis — службы доверия\n\n"
...
def set_webhook():
    """Устанавливает вебхук при первом запуске"""
    webhook_url = f"https://psychobot-xl1y.onrender.com/webhook"
    try:
